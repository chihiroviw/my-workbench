	     ...
	     int frame, int left_right)
{
    ...
    if(left_right==-1){        // for left eye  
        gluLookAt(  4.0+(3.0/5.0)*0.25, 3.0, 3.0-(4.0/5.0)*0.25, 
                    0.0, 2.0, 0.0, 
                    0.0, 1.0, 0.0);   
...
int main(int argc, char** argv)
{
  ...
  while (!glfwWindowShouldClose(Window)){
    ...
    // Calculate brightness of each pixel for the left image
    for(y=0; y<height; y++){
        for(x=0; x<width; x++){
            unsigned char bw = fb_l[(y*width+x)*3+0]*0.299
                             + fb_l[(y*width+x)*3+1]*0.587
                             + fb_l[(y*width+x)*3+2]*0.114;
            
            fb_l[(y*width+x)*3+0] = bw;//r
            fb_l[(y*width+x)*3+1] = bw;//g
            fb_l[(y*width+x)*3+2] = bw;//b
    ...
    // Calculate brightness of each pixel for the right image
    for(y=0; y<height; y++){
      for(x=0; x<width; x++){
            unsigned char bw = fb_r[(y*width+x)*3+0]*0.299
                             + fb_r[(y*width+x)*3+1]*0.587
                             + fb_r[(y*width+x)*3+2]*0.114;
            fb_r[(y*width+x)*3+0] = bw;//r
            fb_r[(y*width+x)*3+1] = bw;//g
            fb_r[(y*width+x)*3+2] = bw;//b           
      }
    }
    ...
    for(y=0; y<height; y++){
        for(x=0; x<width; x++){
            fb_l[(y*width+x)*3+0] = fb_r[(y*width+x)*3+0]/2+128;//r
            //fb_l[(y*width+x)*3+1] = ;//g
            fb_l[(y*width+x)*3+2] = fb_l[(y*width+x)*3+2]/2+128;//b
        }
    }
    ...
    glDrawPixels(width,height,GL_RGB,GL_UNSIGNED_BYTE,fb_l);
    glEnable(GL_DEPTH_TEST);
  
    // output movie file
    if(argc >= 3){
      if(frame<360) make_movie_oneframe(fb_l);
      else if(frame==360){
